...
This notebook documents the steps to go from raw csv's pulled down from INSDC Sequence Read Archive by Rachel to the spreadsheets necessary for GEOME Datathon project management. I'm also going to try to learn to work better in the Tidyverse.
...
5. Set up validation on Boolean columns
4. Freeze up to column F
5. Set up `Metadata_Gathered`validation: (From Author, From Paper)
...
gs4_auth(path = "GEOME_credentials/geome-282803-0475069b49ce.json")
drive_auth(path = "GEOME_credentials/geome-282803-0475069b49ce.json")
...
for(prj in workingBioProjects$project_acc_bioprj[55]){
    ...
    taxonomy_lookup <- tibble(uid = as.numeric(names(taxonomy)), taxonomy = taxonomy) %>%
                              ...
                              add_row(uid=0,name = NA, rank = c("phylum","class","order","family", "genus", "species", "subspecies"), 
                                          id = "0") %>%
                              select(-id) %>% 
                              pivot_wider(names_from=rank, values_from = name, values_fn = list(name = list)) %>%
                              unnest(cols = c(phylum, class,  order, family, genus, species, subspecies, common_name))

    
    taxonomized <- left_join(tibble(uid=namestax$taxonomy_biosamp), taxonomy_lookup, by = "uid") %>%
                    select(genus, species,subspecies,phylum,class,order,family,common_name) %>% replace_na(replace = list(subspecies=""))

    #have to write and overwrite here so I can paste subspecies into infraspecies...
    
    taxonomized <-  add_column(taxonomized, nomenclaturalCode = paste("NCBI queried:",date(), sep = " "), 
                    scientificNameID = namestax$taxonomy_biosamp, 
                    infraspecies = paste(taxonomized$subspecies,namestax$infraspecies_biosamp, sep = " ")) %>%
   ...
   metadata<- bind_cols(prjIDs, namestax[,1:2], taxonomized, otherSRA) %>% 
          ...
          add_column(.after = 9, locality = "", decimalLatitude = "", decimalLongitude = "",                    
                    coordinateUncertaintyInMeters = "", country = "", georeferenceProtocol = "",
          ...
          metadata <- add_column(metadata,.before = 1,sheet_index = str_pad(row.names(metadata),width=4,pad="0"))
  ...
  tryCatch(range_write(ss = gdprj, metadata, sheet = "Samples", range = "A2:BJ4000", col_names = F), 
            error = function(e) { msg <- conditionMessage(e)
                                  write_csv(tibble(index,"attempt_1",msg),"failed.txt", append = T)
                                  try_try_again <<- TRUE
                                  }
            )
  ...
  if(try_try_again) { print("Sleeping for 100 seconds to deal with API quota")
                      ...
                      tryCatch(range_write(ss = gdprj, metadata, sheet = "Samples", range = "A2:BJ4000", col_names = F), 
                                error = function(e) { msg <- conditionMessage(e)
                                  write_csv(tibble(index,"attempt_2",msg),"failed.txt", append = T)
                                  skip_to_next <<- TRUE
                                  }
            ) 
  }
...
nonmarineSRA <- drive_get(path = "GEOME_Datathon/Datathon_Working_Directory/BioProject_Tables/nonMarine_SRA_BioProjects")
...
failed_analysis <- left_join(all5_BioProjects, failed, by = "project_index") %>% replace_na(list(write_result = "Success"))

#failed writes seem to be for datasets with larger than average numbers of runs, samples, but not species....
ggplot(failed_analysis) + geom_boxplot(mapping = aes(x = write_result, y = Runs)) + ylim(c(0,1000))
...
for(index in workingFailed$project_index[6:103]){
    ...
    xlprj <- loadWorkbook(file ="Datathon_BioSample_Template.xlsx", isUnzipped = F)
    ...
    taxonomy_lookup <- tibble(uid = as.numeric(names(taxonomy)), taxonomy = taxonomy) %>%
                              unnest(cols = taxonomy) %>% 
                              add_row(uid=0,name = NA, rank = c("phylum","class","order","family", "genus",
                                                                "species", "subspecies"), 
                                          id = "0") %>%
                              select(-id) %>% 
                              pivot_wider(names_from=rank, values_from = name, values_fn = list(name = list)) %>%
                              unnest(cols = c(phylum, class,  order, family, genus, species, subspecies,
                                              common_name))
  ...
  writeData(wb = cpprj, sheet = "Samples", x = metadata, startCol = 1, startRow = 2, colNames = F )
  
  tryCatch(
  saveWorkbook(wb = cpprj, file = paste0(path, index,"_",prj,".xlsx"), overwrite = TRUE), 
            error = function(e) { msg <- conditionMessage(e)
                                  write_csv(tibble(index,"attempt_1",msg),"failed_excel.txt", append = T)
                                  try_try_again <<- TRUE
                                  }
            )
  ...
  if(try_try_again) {
                      tryCatch(
                        saveWorkbook(wb = cpprj, file = paste0(path, index,"_",prj,".xlsx"), overwrite = TRUE), 
                                error = function(e) { msg <- conditionMessage(e)
                                  write_csv(tibble(index,"attempt_2",msg),"failed_excel.txt", append = T)
                                  skip_to_next <<- TRUE
                                  }
            ) 
  }
...
```{r}
#authorize
gs4_auth(path = "GEOME_credentials/non_service_client_auth.json")
drive_auth(path = "GEOME_credentials/geome-282803-0475069b49ce.json")
...
vegetables_tables <- vegetables_html %>% html_nodes("table") %>% map(html_table) %>% bind_rows()
...
write.csv(vegetables_tables,"NonWildSpecies/domesticatedPlants_Wikipedia_Vegetables.csv", row.names = F)
...
# Human Pathogens
humpathStudents <- read.table("NonWildSpecies/humanPathogen_students_taxized.csv", sep = ",", header = T, stringsAsFactors = F, row.names = 1)
humpathWiki <- read.table("NonWildSpecies/humanPathogen_wikipedia_diseases_eukaryotes.tsv", sep = "\t", header = T, stringsAsFactors = F)
...
domesticatedStudents <- read.csv("NonWildSpecies/domesticated_students_taxized.csv", row.names = 1)

domesticatedSpecies <- full_join(domesticatedWiki, domesticatedStudents, by = c("Species"= "dom"))
...
alldomesticated$Category <- factor(alldomesticated$Category, levels = c("Domesticated Plant Khouri et al. 2016", 
...
```{r}
all <- read.csv(file = "NonWildSpecies/nonWildSpecies_final.tsv", header = T, sep = "\t", stringsAsFactors = F)
...
write.table(all,file = "NonWildSpecies/nonWildSpecies_final_sources.tsv", sep = "\t", quote=F,row.names = F)
```
...
```{r}
m1Betas <- read_delim("models/analysis1_betaTable.csv",delim = " ") %>% 
          #split the CI column
          ...
          mutate(OddsMean = exp(mean), OddsLo95CI = exp(CI95_1), OddsHi95CI = exp(CI95_2))
...
```{r}
m2Betas <- read_delim("models/analysis2_betaTable.csv",delim = " ") %>% 
          #split the CI column
          mutate(CI95 = str_extract_all(`95% CI`,"-*[0-9]\\.[0-9]+")) %>%
          # move the resulting list column into two columns
          unnest_wider(CI95, names_sep = "_") %>% 
...
P(recovered partial data|author response = T) ~ registration date of BioProject
Recovered metadata = T if spatiotemp (collection date AND (coords or locality)) increases from MID to POST

must have gained year OR (locality OR coords) metadata;
any amount of gain counts (e.g. F -> SOME, SOME -> T, F -> MOST, etc.);
inclusion criteria - authors responded and dataset was missing any amount of data in locality or year or coords (after looking in papers)

```{r}
m3Betas <- read_delim("models/analysis3_betaTable.csv",delim = " ") %>% 
          #split the CI column
          mutate(CI95 = str_extract_all(`95% CI`,"-*[0-9]\\.[0-9]+")) %>%
          # move the resulting list column into two columns
          unnest_wider(CI95, names_sep = "_") %>% 
...
gained enough metadata to have year & (locality OR coords) for >= 50% of samples;
only gains that flip metadata from F/SOME to MOST/T count
P(recovered all data|author response = T) ~ registration date of BioProject

```{r}
m4Betas <- read_delim("models/analysis4_betaTable.csv",delim = " ") %>% 
          #split the CI column
          mutate(CI95 = str_extract_all(`95% CI`,"-*[0-9]\\.[0-9]+")) %>%
          # move the resulting list column into two columns
          unnest_wider(CI95, names_sep = "_") %>% 

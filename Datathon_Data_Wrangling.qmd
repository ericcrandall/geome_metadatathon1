...
# Create a phylogenetic tree of datathon species

We don't care about branch lengths, so the easiest approach is to use the Open Tree of Life, via the `rotl` package. Right off the bat we find the problem that `specificEpithet` usually contains the generic name, but not always. I'm going to solve this manually, rather than programatically.
...
ott <- resolved_names %>% drop_na() %>%
                    filter(!str_detect(flags,"incertae_sedis")) %>% 
